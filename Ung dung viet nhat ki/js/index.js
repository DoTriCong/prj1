...
const addArticleClickEvents = (articles) => {
  document.querySelectorAll(".blog-card").forEach((card, index) => {
    card.addEventListener("click", () => {
      ...
      window.location.href = "./pages/article-manager1.html"; 
...
document.addEventListener("DOMContentLoaded", () => {
  const blogContainer = document.querySelector(".blog-container"); // Chọn container hiển thị bài viết
  ...
  articles.forEach(article => {
    ...
    articleCard.innerHTML = `
      <img src="${article.image}" alt="${article.title}" />
      <div class="content">
        <span class="date">Date: ${new Date(article.id).toLocaleDateString()}</span>
        <h2>${article.title}</h2>
        <p>${article.content}</p>
        <img class="iconText" src="./assets/icons/Badge.png" alt="Badge Icon" />
      </div>
    `;

    // Thêm bài viết vào giao diện
    blogContainer.prepend(articleCard); // Thêm bài viết mới nhất lên đầu
  });
});
document.addEventListener("DOMContentLoaded", () => {
  // Lấy thanh tìm kiếm và gắn sự kiện
  const searchInput = document.querySelector(".search-bar");

  searchInput.addEventListener("input", (event) => {
    const keyword = event.target.value.toLowerCase(); // Lấy từ khóa tìm kiếm (chuyển về chữ thường)
    const filteredArticles = articles.filter((article) => {
      return (
        article.title.toLowerCase().includes(keyword) || // Tìm kiếm theo tiêu đề
        article.content.toLowerCase().includes(keyword)  // Tìm kiếm theo nội dung
      );
    });

    currentPage = 1; // Reset về trang đầu
    renderArticles(filteredArticles); // Hiển thị bài viết phù hợp
  });
});
